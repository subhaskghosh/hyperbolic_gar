...
def aggregate_gradients(aggregation_type="average", beta=0):
    ...
    with lock:
        if aggregation_type == "average":
            ...
            gradients_array = np.array([g.flatten() for g in local_gradients])  # Shape (num_nodes, d)
            avg_gradient = hyperbolic_geometric_median_adaptive(gradients_array)
        elif aggregation_type == "curvature_aware_krum":
            avg_gradient = curvature_aware_weighted_krum(local_gradients, beta)
        else:
...
def distributed_sgd(X, y, X_test, y_test, num_nodes, byzantine_nodes, noise_variance, lr=0.01, epochs=10, batch_size=128, dataset="mnist", ipm_attack=False, add_noise=False, aggregation_type="average"):
    ...
    for epoch in range(epochs):
        ...
        # print(f"Epoch {epoch + 1}/{epochs}, Loss: {current_loss:.4f}, Train Acc: {train_accuracy:.2f}%, Test Acc: {test_accuracy:.2f}%")
...
def run_experiments():
    ...
    for dataset, byzantine_fraction, noise_variance, attack_mode, samples_per_node in tqdm(experiment_configs,
        ...
        # Save loss curves
        loss_curve_file = f'loss_curves_1/{dataset}_frac_{byzantine_fraction}_noise_{noise_variance}_attack_{attack_mode}_samples_{samples_per_node}.csv'
        os.makedirs("loss_curves_1", exist_ok=True)
        pd.DataFrame({
            "epoch": range(epochs),
        ...
        os.makedirs("sgd_plots_1", exist_ok=True)
        ...
        plt.savefig(f'sgd_plots_1/{dataset}_frac_{byzantine_fraction}_noise_{noise_variance}_attack_{attack_mode}_samples_{samples_per_node}.pdf',
